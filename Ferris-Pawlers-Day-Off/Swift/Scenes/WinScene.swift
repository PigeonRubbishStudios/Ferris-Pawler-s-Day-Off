...
class WinScene: SKScene
{
    ...
    {
        let winScreenTexture = SKTexture(imageNamed: "winScreen")
        let winScreen = SKSpriteNode(texture: winScreenTexture)
        winScreen.zPosition = -50
        winScreen.position = CGPoint(x: 768, y: 1024)
        winScreen.size = CGSize(width: 1536, height: 2048)
        addChild(winScreen)
    }
    ...
    func createMusic()
    {
        if let musicURL = Bundle.main.url(forResource: "winSong", withExtension: "mp3")
        {
            do
            {
    ...
    func createScores()
    {
        ...
        highScoreLabel.position = CGPoint(x: 20, y: 808.5)
        ...
        yourScoreLabel.position = CGPoint(x: 20, y: 707)
